...
void print_attr_string(NSMutableAttributedString *outString, NSString *str, NSDictionary* current)
{
    NSAttributedString* attrStr = [[[NSAttributedString alloc] initWithString:str attributes:current] autorelease];
    [outString appendAttributedString:attrStr];
}
...
static NSMutableDictionary *merge(NSDictionary *into, NSDictionary *with) {
    ...
    else
    {
        ...
        [result addEntriesFromDictionary:with];
        ...
        if (inheritedParagraphStyle)
        {
            ...
            if (elementParagraphStyle)
            {
                ...
                result[NSParagraphStyleAttributeName] = newParagraphStyle;
            }
        }
...
void print_attr_element_list(NSMutableAttributedString *outStr, element *list, NSDictionary *attributes[], NSDictionary *current) {
    while (list != NULL) {
        print_attr_element(outStr, list, attributes, current);
        list = list->next;
    }
}
...
void print_attr_element(NSMutableAttributedString *outStr, element *elt, NSDictionary *attributes[], NSDictionary *current) {

    switch (elt->key) {
        case SPACE:         print_attr_string(outStr, @" ",current);  break;
        case LINEBREAK:     print_attr_string(outStr, @"\n",current);  break;
        case STRING:        print_attr_string(outStr, elt->contents.str, current);  break;
        case ELLIPSIS:      print_attr_string(outStr, @"\u2026",current); break;
        case EMDASH:        print_attr_string(outStr, @"\u2014",current); break;
        case ENDASH:        print_attr_string(outStr, @"\u2013",current); break;
        case APOSTROPHE:    print_attr_string(outStr, @"\u02BC",current); break;
        case SINGLEQUOTED:
            print_attr_string(outStr, @"\u2018",current);
            print_attr_element_list(outStr, elt->children, attributes, current);
            print_attr_string(outStr, @"\u2019",current);
            break;
        case DOUBLEQUOTED:
            print_attr_string(outStr, @"\u201C",current);
            ...
            break;
        case CODE:
            print_attr_string(outStr, elt->contents.str, merge(current, attributes[elt->key]));
            break;
        case HTML:
        ...
        case LINK:;
            ...
            if (url) {
                ...
                print_attr_element_list(outStr, elt->contents.link->label, attributes, merge(current, merge(attributes[elt->key], linkAttibutes)));
            } else {
                NSDictionary *attributesBroken = @{NSForegroundColorAttributeName: [TARGET_PLATFORM_COLOR redColor]}; // Make this attributes[BROKEN]
                print_attr_element_list(outStr, elt->contents.link->label, attributes, merge(current, attributesBroken));
                print_attr_string(outStr, [NSString stringWithFormat: @" (%@)", elt->contents.link->url], current);
            }
            break;
        case IMAGE:
        ...
        case EMPH: case STRONG:
            print_attr_element_list(outStr, elt->children, attributes, merge(current, attributes[elt->key]));
            break;
        case LIST:
            print_attr_element_list(outStr, elt->children, attributes, merge(current, attributes[elt->key]));
            break;
        ...
        case PARA:
            ...
            print_attr_string(outStr, @"\n",current);
            break;
        case HRULE:         print_attr_string(outStr, @"\n-----------------------------------------------------\n", merge(current, attributes[elt->key])); break;
        case HTMLBLOCK:     print_attr_string(outStr, elt->contents.str, merge(current, attributes[elt->key])); break;
        case VERBATIM:      print_attr_string(outStr, elt->contents.str, merge(current, attributes[elt->key])); break;
        case BULLETLIST:
            padded = 0;
            print_attr_element_list(outStr, elt->children, attributes, merge(current, attributes[elt->key]));
            print_attr_string(outStr, @"\n",current);
...
void print_element_list_attr(NSMutableAttributedString *outStr, element *elt, int exts,NSDictionary *attributes[], NSDictionary *current) {
    /* Initialize globals */
    endnotes = nil;
    notenumber = 0;
    ...
    print_attr_element_list(outStr, elt, attributes, current);
    if (endnotes != nil) {
       // pad(out, 2);
